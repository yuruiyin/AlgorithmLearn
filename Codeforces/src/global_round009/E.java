...
public class E {
    ...
    static class Task {
        ...
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            int n = in.nextInt();
            int[] arr = new int[n];
            Map<Integer, TreeMultiSet<Integer>> indexListMap = new HashMap<>();
            ...
            for (int i = 0; i < n; i++) {
                ...
                if (indexListMap.containsKey(arr[i])) {
                    ...
                    TreeMultiSet<Integer> indexList = new TreeMultiSet<>();
                    indexList.add(i);
                    indexListMap.put(arr[i], indexList);
                }
            ...
            for (int i = 0; i < n - 1; i++) {
                int curNum = arr[i];
                int afterMin = treeMultiSet.lower(curNum);
                treeMultiSet.remove(curNum);
                if (curNum <= afterMin) {
                    indexListMap.get(curNum).pollFirst();
                    continue;
                }

                int u = i + 1;
                int v = indexListMap.get(afterMin).first() + 1;
                ansList.add(new Pair(u, v));
                indexListMap.get(curNum).pollFirst();
                indexListMap.get(curNum).add(v - 1);
                indexListMap.get(afterMin).pollFirst();
                treeMultiSet.remove(afterMin);
                treeMultiSet.add(curNum);
                arr[v - 1] = curNum;
            }

            out.println(ansList.size());
            for (Pair pair : ansList) {
                out.println(pair.u + " " + pair.v);
            }
        }
    }
