...
public class D {
    ...
    static class Task {
        ...
        // 使用单调栈获取右边第一个比当前元素大的元素的索引
        private int[] getLeftLastGeqIdxArr(int[] height) {
            int len = height.length;
            int[] stack = new int[len];
            int stackSize = 0;
            int[] leftLastGeqIdxArr = new int[len];
            Arrays.fill(leftLastGeqIdxArr, -1);
            ...
            for (int i = 0; i < len; i++) {
                ...
                if (stackSize != 0) {
                    leftLastGeqIdxArr[i] = stack[stackSize - 1];
                }
                stack[stackSize++] = i;
            }
            return leftLastGeqIdxArr;
        }
        ...
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            int ansMax = 0;
            while (left <= right) {
                int value = sum - treeMultiSet.last();
                if (arr[left] <= arr[right]) {
                    sum -= arr[left];
                    treeMultiSet.remove(arr[left]);
                    left++;
                } else {
                    sum -= arr[right];
                    treeMultiSet.remove(arr[right]);
                    right--;
                }
