...
public class D {
    ...
    class Data {
        ...
        int time;
        Data(int time, Set<Integer> indexList) {
            this.time = time;
    ...
    public int findMinimumTime(int[][] tasks) {
        ...
        Set<Integer>[] indexListArr = new HashSet[2001];
...
//
//        TreeMultiSet<Data> heap = new TreeMultiSet<>(new Comparator<Data>() {
//            @Override
//            public int compare(Data o1, Data o2) {
//                return o2.indexList.size() - o1.indexList.size();
//            }
//        });
        ...
        for (int i = 1; i <= 2000; i++) {
            Data data = new Data(i, indexListArr[i]);
            dataList.add(data);
            if (data.indexList.size() > maxSize) {
                maxSize = data.indexList.size();
                maxData = data;
            }
        }
        ...
        while (indexSet.size() != len) {
            Set<Integer> indexList = maxData.indexList;
            int time = maxData.time;
            for (int idx : indexList) {
                tasks[idx][2]--;
                if (tasks[idx][2] == 0) {
                    indexSet.add(idx);
                    for (int i = tasks[idx][0]; i <= tasks[idx][1]; i++) {
                        indexListArr[i].remove(idx);
                    }
                }
            }
