...
public class D_1 {
    ...
    private boolean enough(int numIdx, int[] slotStatus) {
        int leftCount = numSlots;
        for (int i = 0; i < numSlots; i++) {
            if (slotStatus[i] == 2) {
                leftCount--;
                if (leftCount * 2 < len - numIdx) {
                    return false;
                }
            }
        }
    ...
    private int dp(int numIdx, int[] slotStatus) {
        if (numIdx == len || !enough(numIdx, slotStatus)) {
            return 0;
        }
    ...
    public static void main(String[] args) {
        System.out.println(new D_1().maximumANDSum(new int[]{1, 2, 3, 4, 5, 6}, 3));
    }
