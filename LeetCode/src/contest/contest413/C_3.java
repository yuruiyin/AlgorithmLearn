...
public class C_3 {
    ...
    private int rec(long pre, long high, long low, int row) {
        if (row == m) {
            // 选到最后一行了
            return 0;
        }
        ...
        // 当前行选或不选
        int nonChooseRes = rec(pre, high, low, row + 1);

        int chooseRes = 0;
        int[] rowArr = this.grid[row];
        int colCount = rowArr.length;
        for (int j = 0; j < colCount; j++) {
            ...
            chooseRes = Math.max(chooseRes, rec(pre | ((long) (j + 1) << (row * 4L)), newHigh, newLow, row + 1) + value);
...
//        System.out.println(new C_3().maxScore(generateGrid()));
        List<List<Integer>> list = new ArrayList<>();
        list.add(new ArrayList<>(Arrays.asList(8, 7, 6)));
        list.add(new ArrayList<>(Arrays.asList(8,3,2)));
        System.out.println(new C_3().maxScore(list));
    }
