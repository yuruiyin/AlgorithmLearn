...
public class E {
    ...
    static class Task {
        ...
        public void solve(int testNumber, InputReader in, PrintWriter out) {
            ...
            TreeMultiSet<Integer>[] treeMultiSets = new TreeMultiSet[MAX];
            int[] belongs = new int[n + 1];
            for (int i = 0; i < n; i++) {
                ratings[i + 1] = in.nextInt();
                int b = in.nextInt();
                if (treeMultiSets[b] == null) {
                    treeMultiSets[b] = new TreeMultiSet<>();
                }

                treeMultiSets[b].add(ratings[i + 1]);
                belongs[i + 1] = b;
            }

            TreeMultiSet<Integer> ratingSet = new TreeMultiSet<>();
            for (int i = 1; i < MAX; i++) {
                if (treeMultiSets[i] == null) {
                    continue;
                }

                ratingSet.add(treeMultiSets[i].last());
            }

            for (int i = 0; i < q; i++) {
                int c = in.nextInt();
                int d = in.nextInt();
                int belong = belongs[c];
                ratingSet.remove(treeMultiSets[belong].last());
                treeMultiSets[belong].remove(ratings[c]);
                if (treeMultiSets[belong] != null && !treeMultiSets[belong].isEmpty()) {
                    ratingSet.add(treeMultiSets[belong].last());
                }

                if (treeMultiSets[d] != null && !treeMultiSets[d].isEmpty()) {
                    ratingSet.remove(treeMultiSets[d].last());
                }
                if (treeMultiSets[d] == null) {
                    treeMultiSets[d] = new TreeMultiSet<>();
                }
                treeMultiSets[d].add(ratings[c]);
                ratingSet.add(treeMultiSets[d].last());

                belongs[c] = d;
                out.println(ratingSet.first());
            }
        }
    }
